...
<body>
    ...
    <div class="weekContainer">
        <div class="calendar">
            ...
            if(!$rezu){}else{$ile=$rezu->num_rows;if($ile>0){echo 'style= "background-color:red"'; }}$conn -> close();}?>>
            ...
            $id=$_SESSION['id'];$rezu=$conn->query("SELECT * FROM jazdy WHERE data_jazdy='$zap'and id_instruktora='$id'");
            if(!$rezu){}else{$ile=$rezu->num_rows;if($ile>0){echo ' style= "background-color:red"'; }}$conn -> close();}?>></a>
            ...
            <a class="record"<?php $conn = @new mysqli($host, $db_user, $db_pass, $db_name);
            if($conn->connect_errno!=0){}else{$zap=date("Y-m-d H:i:s", mktime(6, 0, 0, date("m", $sd), date("d", $sd), date("y", $sd)));
            $id=$_SESSION['id'];$rezu=$conn->query("SELECT * FROM jazdy WHERE data_jazdy='$zap'and id_instruktora='$id'");
            if(!$rezu){}else{$ile=$rezu->num_rows;if($ile>0){echo 'style= "background-color:red"'; }}$conn -> close();}?>></a>
